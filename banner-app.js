...
const POST_SLOTS_AREA = {
    top: 380, bottom: 1230, leftX: 54, rightX: 1026,
    slotH: 130, slotGap: 18, maxSlots: 5
};
...
async function generateFootballListBannerModern(canvasEl, games, bannerNum, totalBanners) {
    ...
    if (isPost) {
        ...
        if (bgImg) {
            ...
            c.fillStyle = '#22c55e';
            c.font = '700 64px Oswald, sans-serif';
            c.textAlign = 'center';
            c.fillText('JOGOS DO DIA', width/2, 200);
        }
    ...
    if (uploadedLogo) {
        ...
        cvs.drawImage(uploadedLogo, width - logoW - 20, 20, logoW, logoSize);
        cvs.restore();
    }
    ...
    for (var gi = 0; gi < games.length; gi++) {
        ...
        cvs.save();
        cvs.fillStyle = 'rgba(255, 255, 255, 0.08)';
        ...
        if (game.homeLogoImg) cvs.drawImage(game.homeLogoImg, leftX - logoSz / 2, logoY - logoSz / 2, logoSz, logoSz);

        cvs.textAlign = 'center';
        cvs.font = '600 15px Manrope, sans-serif';
        cvs.fillStyle = '#fff';
        var homeNameY = logoY + logoSz / 2 + 24;
        cvs.fillText(fitTextToWidth(cvs, game.teams.home.name.toUpperCase(), 200), leftX, homeNameY);

        cvs.textAlign = 'center';
        var centerTextY = currentY + cardHeight / 2;
        cvs.font = '700 16px Manrope, sans-serif';
        cvs.fillStyle = '#ffffff';
        cvs.fillText(fitTextToWidth(cvs, league, 320), centerX, centerTextY - 35);
        cvs.font = '700 38px Oswald, sans-serif';
        cvs.fillText(time, centerX, centerTextY + 10);
        cvs.font = '600 13px Manrope, sans-serif';
        cvs.fillText(fitTextToWidth(cvs, broadcaster, 350), centerX, centerTextY + 40);

        var rightX = width - padding - cardPadding - logoSz / 2 - 120;
        if (game.awayLogoImg) cvs.drawImage(game.awayLogoImg, rightX - logoSz / 2, logoY - logoSz / 2, logoSz, logoSz);
        cvs.font = '600 15px Manrope, sans-serif';
        cvs.fillText(fitTextToWidth(cvs, game.teams.away.name.toUpperCase(), 200), rightX, homeNameY);
...
function drawGameInSlot(c, game, slot) {
    ...
    if (game.homeLogoImg) try { c.drawImage(game.homeLogoImg, homeBlockCx - logoSize / 2, logoTopY, logoSize, logoSize); } catch(e) {}
    if (game.awayLogoImg) try { c.drawImage(game.awayLogoImg, awayBlockCx - logoSize / 2, logoTopY, logoSize, logoSize); } catch(e) {}
...
function loadFootballManualMode() {
    ...
    showPlaceholder('\u270F\uFE0F', 'MANUAL', 'Adicione jogos manualmente', '#0a4d0a', '#001a00');
}
        ...
        '<div class="min-w-0"><label class="text-xs text-zinc-400 block mb-1">Time Casa</label>' +
            '<div class="relative">' +
                '<div class="flex gap-2 items-center min-w-0">' +
                    '<div class="w-10 h-10 border border-zinc-700 rounded flex items-center justify-center shrink-0 overflow-hidden bg-black">' +
...
async function generateTrailerBannerVideo() {
    ...
    try {
        ...
        var fileName = safeTitle + '_trailer_whatsapp.' + finalExt;
